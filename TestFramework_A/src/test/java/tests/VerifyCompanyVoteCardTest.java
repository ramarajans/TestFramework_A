...
public class VerifyCompanyVoteCardTest extends BaseTest{
	 ...
	 * public void verifyCompanyVoteCardTest(String browser, String country) throws
	 * Exception {
	 * 
	 * openBrowser(browser); navigateToURL(DriverManager.getDriver(),
	 * "https://viewpoint.glasslewis.com/WD/?siteId=DemoClient");
	 * companyVoteCardVeri = new VerifyCountryFilterPage(DriverManager.getDriver());
	 * companyVoteCardVeri.checkCountryFilterExist();
	 * countryFilterVeri.enterCountryFieldValue(country);
	 * countryFilterVeri.selectcountryCheckbox(country); Thread.sleep(5000);
	 * countryFilterVeri.clickUpdateButton(); Thread.sleep(5000); boolean
	 * verifyCountry = countryFilterVeri.verifyCountryValuesFiltered(country);
	 * Assert.assertEquals(verifyCountry, true); }
	 * 
	 * @AfterMethod public void afterMethod() {
	 * 
	 * if(DriverManager.getDriver()!=null) { //DriverManager.getDriver().close();
	 * DriverManager.getDriver().quit(); System.out.println("Browser closed"); } }
	 * 
	 * 
	 * @DataProvider(name = "data") public Object[][] dataSupplier() throws
	 * IOException {
	 * 
	 * File file = new File(".//src/test/resources/testdata.xlsx"); FileInputStream
	 ...
	 * 
	 * for (int i = 0; i < lastRowNum; i++) { for (int k = 0; k < lastCellNum; k++)
